...
impl<I: InternalVideoTimeStampRepository, V: InternalVideoRepository>
    TimeStampIndexingService<I, V>
{
    ...
    pub async fn add_or_update_timestamps(
        &self,
        v: &VideoEntity,
        tss: &[VideoTimestampEntity],
    ) -> AppResult<()> {
        if self.ts_repo.find_video_entity_by_id(&v.id).await? {
