...
pub struct MeiliSearchVideoCrudRepository<T: MeiliSearchCrudApi<TimeStampIndex, VideoTimestampEntity> + Send + Sync> {
    client: T,
}
...
impl<T: MeiliSearchCrudApi<TimeStampIndex, VideoTimestampEntity> + Send + Sync> InternalVideoTimeStampRepository
    for MeiliSearchVideoCrudRepository<T>
{
    async fn add_video_timestamp_entity(&self, entity: &VideoTimestampEntity) -> AppResult<()> {
        self.client.add_entity(TimeStampIndex::name(), entity)
            .await
    ...
    async fn add_video_timestamp_entities(&self, entities: &[VideoTimestampEntity]) -> AppResult<()> {
        self.client.add_entities(TimeStampIndex::name(), entities)
            .await
            .map_err(AppError::from)?;
        Ok(())
    }
    ...
    async fn update_video_timestamp_entity(&self, entity: &VideoTimestampEntity) -> AppResult<()> {
        self.client.update_entity(TimeStampIndex::name(), entity)
            .await
            .map_err(AppError::from)?;
        Ok(())
    }
    ...
    async fn update_video_timestamp_entities(&self, entities: &[VideoTimestampEntity]) -> AppResult<()> {
        self.client.update_entities(TimeStampIndex::name(), entities)
            .await
            .map_err(AppError::from)?;
    ...
    async fn find_video_timestamp_entity_by_id(&self, video_id: &TimestampId) -> AppResult<bool> {
        let exists = self.client.find_entity_by_id(TimeStampIndex::name(), video_id.as_str()).await?;
        Ok(exists)
    }

    async fn get_video_timestamp_entity_by_id(&self, video_id: &TimestampId) -> AppResult<Option<VideoTimestampEntity>> {
        let entity = self.client.get_entity_by_id(TimeStampIndex::name(), video_id.as_str()).await?;
        Ok(entity)
    }
    ...
    async fn delete_video_timestamp_entity_by_id(&self, video_id: &VideoId) -> AppResult<()> {
        self.client.delete_entity_by_id(TimeStampIndex::name(), video_id.as_str())
            .await
            .map_err(AppError::from)?;
        Ok(())
    }
    ...
    async fn delete_all_video_timestamp_entities(&self) -> AppResult<()> {
        self.client.delete_all_entities(TimeStampIndex::name())
            .await
            .map_err(AppError::from)?;
        Ok(())
    }
